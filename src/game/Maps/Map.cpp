...
void Map::AddTransport(Transport* transport)
{
    ...
    m_infiniteObjects.insert(transport);
}
...
void Map::RemoveTransport(Transport* transport)
{
    ...
    m_infiniteObjects.erase(transport);
}
...
bool Map::IsUpdateObjectTick() const
{
...
void Map::CreatePlayerOnClient(Player* player)
{
    ...
    SendInitInfiniteObjects(player, updateData);
...
bool Map::Add(Player* player)
{
    ...
    if (sWorld.getConfig(CONFIG_BOOL_SOLOCRAFT_ENABLED))
    {
        ...
        dungeons =
        {
            ...
            //WotLK Instances
            {533, sWorld.getConfig(CONFIG_UINT32_STRATHOLMERAID_LEVEL) },                      // Naxxramas
            {574, sWorld.getConfig(CONFIG_UINT32_VALGARDE70_LEVEL) },                          // Utgarde Keep
            {575, sWorld.getConfig(CONFIG_UINT32_UTGARDEPINNACLE_LEVEL) },
            {576, sWorld.getConfig(CONFIG_UINT32_NEXUS70_LEVEL) },                             // The Nexus
            {578, sWorld.getConfig(CONFIG_UINT32_NEXUS80_LEVEL) },                             // The Occulus
            {595, sWorld.getConfig(CONFIG_UINT32_STRATHOLMECOT_LEVEL) },                       // The Culling of Stratholme
            {599, sWorld.getConfig(CONFIG_UINT32_ULDUAR70_LEVEL) },                            // Halls of Stone
            {600, sWorld.getConfig(CONFIG_UINT32_DRAKTHERONKEEP_LEVEL) },                      // Drak'Tharon Keep
            {601, sWorld.getConfig(CONFIG_UINT32_AZJOL_UPPERCITY_LEVEL) },                     // Azjol-Nerub
            {602, sWorld.getConfig(CONFIG_UINT32_ULDUAR80_LEVEL) },                            // Halls of Lighting
            {603, sWorld.getConfig(CONFIG_UINT32_ULDUARRAID_LEVEL) },                          // Ulduar
            {604, sWorld.getConfig(CONFIG_UINT32_GUNDRAK_LEVEL) },
            {608, sWorld.getConfig(CONFIG_UINT32_DALARANPRISON_LEVEL) },                       // Violet Hold
            {615, sWorld.getConfig(CONFIG_UINT32_CHAMBEROFASPECTSBLACK_LEVEL) },               // The Obsidian Sanctum
            {616, sWorld.getConfig(CONFIG_UINT32_NEXUSRAID_LEVEL) },                           // The Eye of Eternity
            {619, sWorld.getConfig(CONFIG_UINT32_AZJOL_LOWERCITY_LEVEL) },                     // Ahn'kahet: The Old Kingdom
            {631, sWorld.getConfig(CONFIG_UINT32_ICECROWNCITADEL_LEVEL) },                     // Icecrown Citadel
            {632, sWorld.getConfig(CONFIG_UINT32_ICECROWNCITADEL5MAN_LEVEL) },                 // The Forge of Souls
            {649, sWorld.getConfig(CONFIG_UINT32_ARGENTTOURNAMENTRAID_LEVEL) },                // Trial of the Crusader
            {650, sWorld.getConfig(CONFIG_UINT32_ARGENTTOURNAMENTDUNGEON_LEVEL) },             // Trial of the Champion
            {658, sWorld.getConfig(CONFIG_UINT32_QUARRYOFTEARS_LEVEL) },                       // Pit of Saron
            {668, sWorld.getConfig(CONFIG_UINT32_HALLSOFREFLECTION_LEVEL) },                   // Halls of Reflection
            {724, sWorld.getConfig(CONFIG_UINT32_CHAMBEROFASPECTSRED_LEVEL) },                 // The Ruby Sanctum
        };
        // Dungeon Difficulty
        // Catch alls
        ...
        diff_Multiplier =
        {
            ...
            // WOTLK Instances
            {533, sWorld.getConfig(CONFIG_FLOAT_STRATHOLMERAID_DIFF) },                        //  Nax 10
            {574, sWorld.getConfig(CONFIG_FLOAT_VALGARDE70_DIFF) },                             // Utgarde Keep
            {575, sWorld.getConfig(CONFIG_FLOAT_UTGARDEPINNACLE_DIFF) },
            {576, sWorld.getConfig(CONFIG_FLOAT_NEXUS70_DIFF) },                                // The Nexus
            {578, sWorld.getConfig(CONFIG_FLOAT_NEXUS80_DIFF) },                                // The Occulus
            {595, sWorld.getConfig(CONFIG_FLOAT_STRATHOLMECOT_DIFF) },                          // The Culling of Stratholme
            {599, sWorld.getConfig(CONFIG_FLOAT_ULDUAR70_DIFF) },                               // Halls of Stone
            {600, sWorld.getConfig(CONFIG_FLOAT_DRAKTHERONKEEP_DIFF) },                         // Drak'Tharon Keep
            {601, sWorld.getConfig(CONFIG_FLOAT_AZJOL_UPPERCITY_DIFF) },                        // Azjol-Nerub
            {602, sWorld.getConfig(CONFIG_FLOAT_ULDUAR80_DIFF) },                               // Halls of Lighting
            {603, sWorld.getConfig(CONFIG_FLOAT_ULDUARRAID_DIFF) },                            // Ulduar 10
            {604, sWorld.getConfig(CONFIG_FLOAT_GUNDRAK_DIFF) },
            {608, sWorld.getConfig(CONFIG_FLOAT_DALARANPRISON_DIFF) },                          // Violet Hold
            {615, sWorld.getConfig(CONFIG_FLOAT_CHAMBEROFASPECTSBLACK_DIFF) },                 // The Obsidian Sanctum 10
            {616, sWorld.getConfig(CONFIG_FLOAT_NEXUSRAID_DIFF) },                             // The Eye of Eternity 10
            {619, sWorld.getConfig(CONFIG_FLOAT_AZJOL_LOWERCITY_DIFF) },                        // Ahn'kahet: The Old Kingdom
            {631, sWorld.getConfig(CONFIG_FLOAT_ICECROWNCITADEL_DIFF) },                       // Icecrown Citadel 10
            {632, sWorld.getConfig(CONFIG_FLOAT_ICECROWNCITADEL5MAN_DIFF) },                    // The Forge of Souls
            {649, sWorld.getConfig(CONFIG_FLOAT_ARGENTTOURNAMENTRAID_DIFF) },                  // Trial of the Crusader 10
            {650, sWorld.getConfig(CONFIG_FLOAT_ARGENTTOURNAMENTDUNGEON_DIFF) },                // Trial of the Champion
            {658, sWorld.getConfig(CONFIG_FLOAT_QUARRYOFTEARS_DIFF) },                          // Pit of Saron
            {668, sWorld.getConfig(CONFIG_FLOAT_HALLSOFREFLECTION_DIFF) },                      // Halls of Reflection
            {724, sWorld.getConfig(CONFIG_FLOAT_CHAMBEROFASPECTSRED_DIFF) },                   // The Ruby Sanctum 10
        };
        // Heroics
        diff_Multiplier_Heroics =
        {
            ...
            {533, sWorld.getConfig(CONFIG_FLOAT_HEROIC_STRATHOLMERAID_DIFF) },                       // Naxxramas 25
            {574, sWorld.getConfig(CONFIG_FLOAT_HEROIC_VALGARDE70_DIFF) },                            // Utgarde Keep H
            {575, sWorld.getConfig(CONFIG_FLOAT_HEROIC_UTGARDEPINNACLE_DIFF) },                       // Utgarde Pinnacle H
            {576, sWorld.getConfig(CONFIG_FLOAT_HEROIC_NEXUS70_DIFF) },                               // The Nexus H
            {578, sWorld.getConfig(CONFIG_FLOAT_HEROIC_NEXUS80_DIFF) },                               // The Occulus H
            {595, sWorld.getConfig(CONFIG_FLOAT_HEROIC_STRATHOLMECOT_DIFF) },                         // The Culling of Stratholme H
            {599, sWorld.getConfig(CONFIG_FLOAT_HEROIC_ULDUAR70_DIFF) },                              // Halls of Stone H
            {600, sWorld.getConfig(CONFIG_FLOAT_HEROIC_DRAKTHERONKEEP_DIFF) },                        // Drak'Tharon Keep H
            {601, sWorld.getConfig(CONFIG_FLOAT_HEROIC_AZJOL_UPPERCITY_DIFF) },                       // Azjol-Nerub H
            {602, sWorld.getConfig(CONFIG_FLOAT_HEROIC_ULDUAR80_DIFF) },                              // Halls of Lighting H
            {603, sWorld.getConfig(CONFIG_FLOAT_HEROIC_ULDUARRAID_DIFF) },                           // Ulduar 25
            {604, sWorld.getConfig(CONFIG_FLOAT_HEROIC_GUNDRAK_DIFF) },                               // Gundrak H
            {608, sWorld.getConfig(CONFIG_FLOAT_HEROIC_DALARANPRISON_DIFF) },                         // Violet Hold H
            {615, sWorld.getConfig(CONFIG_FLOAT_HEROIC_CHAMBEROFASPECTSBLACK_DIFF) },                // The Obsidian Sanctum 25
            {616, sWorld.getConfig(CONFIG_FLOAT_HEROIC_NEXUSRAID_DIFF) },                            // The Eye of Eternity 25
            {619, sWorld.getConfig(CONFIG_FLOAT_HEROIC_AZJOL_LOWERCITY_DIFF) },                       // Ahn'kahet: The Old Kingdom H
            {631, sWorld.getConfig(CONFIG_FLOAT_HEROIC_ICECROWNCITADEL_DIFF) },                      // Icecrown Citadel 25
            {632, sWorld.getConfig(CONFIG_FLOAT_HEROIC_ICECROWNCITADEL5MAN_DIFF) },                   // The Forge of Souls
            {649, sWorld.getConfig(CONFIG_FLOAT_HEROIC_ARGENTTOURNAMENTRAID_DIFF) },                 // Trial of the Crusader 25
            {650, sWorld.getConfig(CONFIG_FLOAT_HEROIC_ARGENTTOURNAMENTDUNGEON_DIFF) },               // Trial of the Champion H
            {658, sWorld.getConfig(CONFIG_FLOAT_HEROIC_QUARRYOFTEARS_DIFF) },                         // Pit of Saron H
            {668, sWorld.getConfig(CONFIG_FLOAT_HEROIC_HALLSOFREFLECTION_DIFF) },                     // Halls of Reflection H
            {724, sWorld.getConfig(CONFIG_FLOAT_HEROIC_CHAMBEROFASPECTSRED_DIFF) },                  // The Ruby Sanctum 25
        };

        //Unique Raids beyond the heroic and normal versions of themselves
        D649H10 = sWorld.getConfig(CONFIG_FLOAT_HEROIC_ARGENTTOURNAMENTRAID10_DIFF);
        D649H25 = sWorld.getConfig(CONFIG_FLOAT_HEROIC_ARGENTTOURNAMENTRAID25_DIFF);  //Trial of the Crusader 25 Heroic
...
void Map::CorpseRelocation(Corpse* corpse, float x, float y, float z, float orientation)
{
    ...
    AddUpdateMovementObject(corpse);
}
...
void Map::SendInitInfiniteObjects(Player* player, UpdateData& updateData) const
{
...
void Map::SendRemoveInfinite(Player* player) const
{
    if (m_infiniteObjects.size() == 0)
        return;
...
void Map::UpdateVisibility(UpdateDataMapType& update_players)
{
    ...
    if (m_clientUpdateTick % 3 == 0) // every 1200ms update vis on moved objects
    {
        ...
        for (auto& movObj : movementObjects)
        {
            ...
            {
                movObj->UpdateVisibility(update_players);
                visited.insert(movObj);
            }
        }
    }
    ...
    if (IsBattleGroundOrArena() ? IsUpdateObjectTick() : IsStealthTick())
    {
        for (m_mapRefIter = m_mapRefManager.begin(); m_mapRefIter != m_mapRefManager.end(); ++m_mapRefIter)
        {
            Player* plr = m_mapRefIter->getSource();
...
void Map::AddCameraToWorld(WorldObject* obj)
{
    CellPair p = MaNGOS::ComputeCellPair(obj->GetPositionX(), obj->GetPositionY());
    Cell cell(p);
    NGridType* grid = getNGrid(cell.GridX(), cell.GridY());
    obj->GetViewPoint().Event_AddedToWorld(&(*grid)(cell.CellX(), cell.CellY()), nullptr);
}
