...
void Unit::Update(const uint32 diff)
{
    ...
    if (CanHaveThreatList() && IsThreatUpdateSent())
...
uint32 Unit::DealDamage(Unit* dealer, Unit* victim, uint32 damage, CleanDamage const* cleanDamage, DamageEffectType damagetype, SpellSchoolMask damageSchoolMask, SpellEntry const* spellInfo, bool durabilityLoss, Spell* spell)
{
    ...
    if (dealer)
    {
        ...
        for (auto vShareDamageAura : vShareDamageAuras)
        {
            ...
            if (spellInfo && spellInfo->Effect[EFFECT_INDEX_0] == SPELL_EFFECT_APPLY_AURA &&
                spellInfo->EffectApplyAuraName[EFFECT_INDEX_0] == SPELL_AURA_SHARE_DAMAGE_PCT)
                break;
        ...
        if (dealer->IsPlayer() && dealer != victim)
        {
            ...
            if (victim->IsPlayer() && (!spellInfo || !spellInfo->HasAttribute(SPELL_ATTR_EX7_DO_NOT_COUNT_FOR_PVP_SCOREBOARD)))
            {
...
void Unit::Kill(Unit* killer, Unit* victim, DamageEffectType damagetype, SpellEntry const* spellInfo, bool durabilityLoss, bool duel_hasEnded)
{
    ...
    */
    if (victim->IsPlayer())          // Killed player
    {
        ...
        if (responsiblePlayer && (!spellInfo || !spellInfo->HasAttribute(SPELL_ATTR_EX7_DO_NOT_LOG_PVP_KILL))) // PvP kill
        {
...
void Unit::HandleDamageDealt(Unit* dealer, Unit* victim, uint32& damage, CleanDamage const* cleanDamage, DamageEffectType damagetype, SpellSchoolMask damageSchoolMask, SpellEntry const* spellInfo, bool duel_hasEnded)
{
    ...
    if (dealer)
    {
        if (victim->CanAttack(dealer) && (!spellInfo || !spellInfo->HasAttribute(SPELL_ATTR_EX_NO_THREAT)) && dealer->CanEnterCombat() && victim->CanEnterCombat())
        {
            ...
            if (damagetype != DOT && damagetype != SPELL_DAMAGE_SHIELD && (!spellInfo || !spellInfo->HasAttribute(SPELL_ATTR_EX4_REACTIVE_DAMAGE_PROC))) // DOTs dont put in combat but still cause threat
            {
...
void Unit::DealSpellDamage(Unit* affectiveCaster, SpellNonMeleeDamage* spellDamageInfo, bool durabilityLoss, bool resetLeash)
{
    ...
    CleanDamage cleanDamage(spellDamageInfo->damage, BASE_ATTACK, spellDamageInfo->HitInfo & SPELL_HIT_TYPE_CRIT ? MELEE_HIT_CRIT : MELEE_HIT_NORMAL, (spellDamageInfo->damage > 0 || spellDamageInfo->absorb > 0), 0);
...
void Unit::CalculateMeleeDamage(Unit* pVictim, CalcDamageInfo* calcDamageInfo, WeaponAttackType attackType /*= BASE_ATTACK*/)
{
    ...
    if (this != pVictim && IsPlayer() && (GetPowerType() == POWER_RAGE))
    {
        uint32 weaponSpeedHitFactor = 0;
        uint32 damage = 0;

        switch (attackType)
        {
            case BASE_ATTACK: {
                if (calcDamageInfo->hitOutCome == MELEE_HIT_CRIT)
                    weaponSpeedHitFactor = uint32(GetAttackTime(attackType) / 1000.0f * 7);
                else
                    weaponSpeedHitFactor = uint32(GetAttackTime(attackType) / 1000.0f * 3.5f);

                damage = calcDamageInfo->cleanDamage;
                break;
            }
            case OFF_ATTACK: {
                if (calcDamageInfo->hitOutCome == MELEE_HIT_CRIT)
                    ...
                    weaponSpeedHitFactor = uint32(GetAttackTime(attackType) / 1000.0f * 1.75f);
        ...
        {
            float addRage;
            float rageconversion = static_cast<Player*>(this)->CalculateRageConversion();
            ...
            calcDamageInfo->rageGained = addRage * 10;
            if (calcDamageInfo->rageGained > 0)
                calcDamageInfo->HitInfo |= HITINFO_RAGE_GAIN;
        }
    }
}
...
void Unit::DealMeleeDamage(CalcDamageInfo* calcDamageInfo, bool durabilityLoss)
{
    ...
    CleanDamage cleanDamage(calcDamageInfo->cleanDamage, calcDamageInfo->attackType, calcDamageInfo->hitOutCome, calcDamageInfo->totalDamage || calcDamageInfo->absorb, calcDamageInfo->rageGained);
...
void Unit::CalculateDamageAbsorbAndResist(Unit* caster, SpellSchoolMask schoolMask, DamageEffectType damagetype, const uint32 damage, uint32* absorb, int32* resist, bool canReflect, bool canResist, bool binary)
{
    ...
    if (caster && caster != this)
    {
        ...
        for (AuraList::const_iterator i = vSplitDamageFlat.begin(), next; i != vSplitDamageFlat.end() && remainingDamage >= 0; i = next)
        {
            ...
            CleanDamage cleanDamage(splitted, BASE_ATTACK, MELEE_HIT_NORMAL, splitted > 0, 0);
...
void Unit::CalculateAbsorbResistBlock(Unit* caster, SpellNonMeleeDamage* spellDamageInfo, SpellEntry const* spellInfo, WeaponAttackType attType)
{
    ...
    uint32 absorb_affected_damage = caster ? caster->CalcNotIgnoreAbsorbDamage(spellDamageInfo->damage, GetSpellSchoolMask(spellInfo), spellInfo) : spellDamageInfo->damage;
    CalculateDamageAbsorbAndResist(caster, spellDamageInfo->schoolMask, SPELL_DIRECT_DAMAGE, absorb_affected_damage, &spellDamageInfo->absorb, &spellDamageInfo->resist, IsReflectableSpell(spellInfo), IsResistableSpell(spellInfo) && !spellInfo->HasAttribute(SPELL_ATTR_EX5_NO_PARTIAL_RESISTS), IsBinarySpell(*spellInfo));
...
{
    Die<UnitCombatDieSide, UNIT_COMBAT_DIE_HIT, NUM_UNIT_COMBAT_DIE_SIDES> die;
    if (!spellInfo->HasAttribute(SPELL_ATTR_EX7_NO_ATTACK_MISS))
...
{
    Die<UnitCombatDieSide, UNIT_COMBAT_DIE_HIT, NUM_UNIT_COMBAT_DIE_SIDES> die;
    die.set(UNIT_COMBAT_DIE_MISS, CalculateSpellMissChance(pVictim, schoolMask, spellInfo));
    die.set(UNIT_COMBAT_DIE_RESIST, CalculateSpellResistChance(pVictim, schoolMask, spellInfo));
    if (pVictim->CanDeflectAbility(this, spellInfo))
        die.set(UNIT_COMBAT_DIE_DEFLECT, pVictim->CalculateAbilityDeflectChance(this, spellInfo));
    DEBUG_FILTER_LOG(LOG_FILTER_COMBAT, "MagicSpellHitResult: New spell hit die: %u [MISS:%u, RESIST:%u, DEFLECT:%u]", spellInfo->Id,
...
void Unit::SendAttackStateUpdate(CalcDamageInfo* calcDamageInfo) const
{
    ...
    if (calcDamageInfo->HitInfo & HITINFO_RAGE_GAIN)
        data << uint32(calcDamageInfo->rageGained);         // count of some sort?
...
bool Unit::IsHostileTo(Unit const* unit) const
{
    ...
    if (tester->GetTypeId() == TYPEID_PLAYER && target->GetTypeId() == TYPEID_PLAYER)
    {
        ...
        // Sanctuary
        if (pTarget->HasFlag(PLAYER_FLAGS, AREA_FLAG_SANCTUARY) && pTester->HasFlag(PLAYER_FLAGS, AREA_FLAG_SANCTUARY))
            return false;
...
int32 Unit::DealHeal(Unit* pVictim, uint32 addhealth, SpellEntry const* spellInfo, bool critical, uint32 absorb)
{
    ...
    unit->SendHealSpellLog(pVictim, spellInfo->Id, addhealth, addhealth - gain, critical, absorb);
    ...
    if (unit->IsPlayer())
    {
        ...
        if (gain)
            static_cast<Player*>(unit)->GetAchievementMgr().UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_HEALING_DONE, gain, 0, pVictim);

        static_cast<Player*>(unit)->GetAchievementMgr().UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_HIGHEST_HEAL_CASTED, addhealth);
    }

    if (pVictim->IsPlayer())
    {
        static_cast<Player*>(pVictim)->GetAchievementMgr().UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_TOTAL_HEALING_RECEIVED, gain);
        static_cast<Player*>(pVictim)->GetAchievementMgr().UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_HIGHEST_HEALING_RECEIVED, addhealth);
    }
 ...
 */
int32 Unit::SpellBonusWithCoeffs(SpellEntry const* spellProto, SpellEffectIndex effectIndex, int32 total, int32 benefit, int32 ap_benefit, bool donePart, float defCoeffMod)
{
...
uint32 Unit::SpellDamageBonusTaken(Unit* caster, SpellSchoolMask schoolMask, SpellEntry const* spellInfo, SpellEffectIndex effectIndex, uint32 pdamage, DamageEffectType damagetype, uint32 stack)
{
    ...
    TakenTotalMod *= GetTotalAuraMultiplierByMiscValueForMask(SPELL_AURA_MOD_MECHANIC_DAMAGE_TAKEN_PERCENT, GetAllSpellMechanicMask(spellInfo));
...
uint32 Unit::SpellHealingBonusDone(Unit* victim, SpellEntry const* spellInfo, SpellEffectIndex effectIndex, int32 healamount, DamageEffectType damagetype, uint32 stack)
{
    ...
    // apply ap bonus and benefit affected by spell power implicit coeffs and spell level penalties
    DoneTotal = SpellBonusWithCoeffs(spellInfo, effectIndex, DoneTotal, DoneAdvertisedBenefit, 0, true, SCALE_SPELLPOWER_HEALING);
...
uint32 Unit::SpellHealingBonusTaken(Unit* caster, SpellEntry const* spellInfo, SpellEffectIndex effectIndex, int32 healamount, DamageEffectType damagetype, uint32 stack)
{
    ...
    // apply benefit affected by spell power implicit coeffs and spell level penalties
    TakenTotal = caster->SpellBonusWithCoeffs(spellInfo, effectIndex, TakenTotal, TakenAdvertisedBenefit, 0, false, SCALE_SPELLPOWER_HEALING);
...
uint32 Unit::MeleeDamageBonusDone(Unit* victim, uint32 pdamage, WeaponAttackType attType, SpellSchoolMask schoolMask, SpellEntry const* spellInfo, SpellEffectIndex effectIndex, DamageEffectType damagetype, uint32 stack, bool flat)
{
    ...
    {
        if (!spellInfo || !spellInfo->HasAttribute(SPELL_ATTR_EX6_IGNORE_CASTER_DAMAGE_MODIFIERS))
        {
            AuraList const& mModDamagePercentDone = GetAurasByType(SPELL_AURA_MOD_DAMAGE_PERCENT_DONE);
            for (auto i : mModDamagePercentDone)
            {
                if (i->GetModifier()->m_miscvalue & schoolMask &&                         // schoolmask has to fit with the intrinsic spell school
                    i->GetModifier()->m_miscvalue & spellInfo->SchoolMask &&              // AND schoolmask has to fit with weapon damage school (essential for non-physical spells)
                    ((i->GetSpellProto()->EquippedItemClass == -1) ||                     // general, weapon independent
                        (pWeapon && pWeapon->IsFitToSpellRequirements(i->GetSpellProto()))))  // OR used weapon fits aura requirements
                {
                    DoneTotalMod *= (i->GetModifier()->m_amount + 100.0f) / 100.0f;
...
#ifdef BUILD_ELUNA
                else if (elunaMapEvents || elunaWorldEvents)
                {
...
int32 Unit::CalculateAuraDuration(SpellEntry const* spellInfo, uint32 effectMask, int32 duration, Unit const* caster)
{
    ...
    {
        dispelMod   = GetTotalAuraModifierByMiscValue(SPELL_AURA_MOD_DURATION_OF_EFFECTS_BY_DISPEL, spellInfo->Dispel);
        dmgClassMod = GetTotalAuraModifierByMiscValue(SPELL_AURA_MOD_DURATION_OF_MAGIC_EFFECTS, spellInfo->DmgClass);
    }
...
bool Unit::IsIgnoreUnitState(SpellEntry const* spellInfo, IgnoreUnitState ignoreState) const
{
    ...
    for (auto stateAura : stateAuras)
    {
        if (stateAura->GetModifier()->m_miscvalue == ignoreState)
        {
            ...
            if (stateAura->isAffectedOnSpell(spellInfo))
                return true;
        }
    }
