...
void WorldSession::SendStablePet(ObjectGuid guid) const
{
    ...
    else
    {
        ...
        {
            auto queryResult = CharacterDatabase.PQuery("SELECT owner, id, entry, level, name FROM character_pet WHERE owner = '%u' AND slot = '%u' ORDER BY slot",
    ...
    //                                                  0      1     2   3      4      5
    auto queryResult = CharacterDatabase.PQuery("SELECT owner, slot, id, entry, level, name FROM character_pet WHERE owner = '%u' AND slot >= '%u' AND slot <= '%u' ORDER BY slot",
